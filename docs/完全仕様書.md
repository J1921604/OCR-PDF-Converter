...
1. **PDFアップロード**: ブラウザから直接PDFファイルをアップロード（50MB以下）
2. **OCR処理**: Pythonバックエンドで複数OCRエンジン（OnnxOCR 2025.5、PaddleOCR 2.7.0.3）を並列実行し、最高精度の結果を自動選択
3. **テキストレイヤー生成**: 最良エンジンの結果を元のPDFに透明レイヤーとして埋め込み
...
| OCRエンジン1 | OnnxOCR (Python) | 2025.5 |
...
### 1.3.1 OCRエンジン並列処理アーキテクチャ

本システムは、複数OCRエンジンを並列実行し、最高精度の結果を自動選択する独自機構を実装：

**処理フロー**:
1. UIで複数エンジン選択（OnnxOCR、PaddleOCRのチェックボックス）
2. 各PDFページで選択された全エンジンを並列OCR実行
3. 各エンジンの平均信頼度（confidence）を計算
4. 最も高い平均信頼度を持つエンジンの結果を採用
5. 採用された結果で透明テキストレイヤーを生成

**実装位置**: [backend/main.py L630-L672](https://github.com/J1921604/OCR-PDF-Converter/blob/main/backend/main.py#L630-L672)
...
- [OnnxOCR Documentation](https://github.com/hiroi-sora/Umi-OCR)
...
| OnnxOCR | 高速CPU推論OCRエンジン（PaddleOCRベース） |
| PaddleOCR | 高精度多機能OCRエンジン |
