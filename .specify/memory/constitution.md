...
**プロジェクト名**: OCR検索可能PDF変換Webアプリケーション  
...
本プロジェクトは、スキャンPDFを高精度OCR処理し、検索可能なテキストレイヤーを埋め込むWebアプリケーションです。

**主要技術**:
- **フロントエンド**: React 18.2 + Webpack 5.104
- **バックエンド**: Python 3.10.11 + Flask 3.0
- **OCRエンジン**:
  - **OnnxOCR 2025.5**: 高速CPU推論（PaddleOCRベース）
  - **PaddleOCR 2.7.0.3**: 高精度日本語特化モデル
- **PDF処理**: pypdfium2 4.30 + pypdf 5.1 + ReportLab 4.2

### 複数エンジン並列処理機構

独自の高精度OCR実現機構:
1. UIでOnnxOCR、PaddleOCRから複数選択可能
2. 各PDFページで全選択エンジンを並列OCR実行
3. 各エンジンの平均信頼度（confidence）を計算
4. 最も高い平均信頼度を持つエンジンの結果を自動採用
5. 採用された結果で透明テキストレイヤーを生成

**実装**: [backend/main.py L630-L672](https://github.com/J1921604/OCR-PDF-Converter/blob/main/backend/main.py#L630-L672)
...
**Version**: 1.0.0 | **Ratified**: 2026-1-15 | **Last Amended**: 2026-1-15
